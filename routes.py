    ...
    if args["searchForm"].validate_on_submit():
        ...
        if "results" in args["api_response"]:
            for result in args["api_response"]["results"]:
                if "name" in result:
                    formatted_results[result["id"]] = {"name": result["name"], "picture": result["picture"]}
                    if "locations" in result:
                        formatted_locations = {}
                        alternate_locations = ""
                        for location in result["locations"]:
                            if location["url"]:
                                formatted_locations[location["id"]] = {"url": location["url"], "icon": location["icon"], "display_name": location["display_name"]}
                            else:
        ...
        return displayPage("search_results_2")
    return displayPage("home", False)

@app.route("/about")
def about():
    args["title"] = "About"
    return displayPage("about", False)
...
def google_login():
    ...
    if "id_token" in req_data:
        ...
        if isVerified:
            OAuthController.checkExistingUser(isVerified["user_id"], isVerified["user_email"], isVerified["user_token"])
...
def register():
    ...
    if args["registerForm"].validate_on_submit():
        ...
        if UserController.createNewUser("auto_gen", name, args["registerForm"].email.data, args["registerForm"].password.data, "Website"):
